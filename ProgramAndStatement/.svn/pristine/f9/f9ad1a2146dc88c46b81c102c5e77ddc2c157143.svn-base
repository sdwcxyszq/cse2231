...
public class Statement2 extends StatementSecondary {
    ...
    private void createNewRep() {
        ...
        Sequence<Tree<StatementLabel>> children = new Sequence1L<>();
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        Statement2 Local_s = (Statement2) s;
        Sequence<Tree<StatementLabel>> q = new Sequence1L<>();
        StatementLabel root = this.rep.disassemble(q);
        q.add(pos, Local_s.rep);
        this.rep.assemble(root, q);
    }
    ...
    public final Statement removeFromBlock(int pos) {
         ...
         * code.
         */
        Statement2 remove = this.newInstance();
        Sequence<Tree<StatementLabel>> q = new Sequence1L<>();
        StatementLabel root = this.rep.disassemble(q);
        remove.rep = q.remove(pos);
        return remove;
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        Statement2 IF = (Statement2) s1;
        Statement2 ELSE = (Statement2) s2;
        StatementLabel label = new StatementLabel(Kind.IF_ELSE, c);
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        children.add(0, IF.rep);
        children.add(1, ELSE.rep);
        this.rep.assemble(label, children);
        IF.createNewRep();
        ELSE.createNewRep();
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        Statement2 IF = (Statement2) s1;
        Statement2 ELSE = (Statement2) s2;
        Sequence<Tree<StatementLabel>> q = this.rep.newSequenceOfTree();
        StatementLabel root = this.rep.disassemble(q);
        IF.rep = q.remove(0);
        ELSE.rep = q.remove(0);
        this.createNewRep();
    ...
    public final Condition disassembleWhile(Statement s) {
        ...
        Statement2 WHILE = (Statement2) s;
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        StatementLabel root = this.rep.disassemble(children);
        this.createNewRep();
        WHILE.rep = children.remove(0);
        return root.condition;
    }
