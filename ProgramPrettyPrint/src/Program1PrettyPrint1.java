...
public final class Program1PrettyPrint1 extends Program1 {
    ...
    public void prettyPrint(SimpleWriter out) {
        ...
        Map<String, Statement> text = this.replaceContext(temp);
        for (Map.Pair<String, Statement> pair : text) {
            out.println();
            printSpaces(out, INDENT_SIZE);
            out.println("INSTRUCTION " + pair.key() + " IS");
            pair.value().prettyPrint(out, 2 * INDENT_SIZE);
            printSpaces(out, INDENT_SIZE);
            out.println("END " + pair.key());
        }
        this.replaceContext(text);
        Statement state1 = this.newBody();
        Statement body = this.replaceBody(state1);
        out.println();
        out.println("BEGIN");
