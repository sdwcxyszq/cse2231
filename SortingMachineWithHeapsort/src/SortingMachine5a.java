 ...
 * </pre>
 *
 * @author Nicholas Shepard & Ziqi Sheng
 *
 */
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        if (leftIndex < last && rightIndex < last) {
            if (array.mayBeExamined(leftIndex)
                    && array.mayBeExamined(rightIndex)) {
                T leftNode = array.entry(leftIndex);
                T rightNode = array.entry(rightIndex);

                if (order.compare(leftNode, rightNode) < 0) {
                    if (order.compare(root, rightNode) <= 0) {
                        array.exchangeEntries(top, rightIndex);
                        siftDown(array, rightIndex, leftIndex, order);
                    } else if (order.compare(root, leftNode) <= 0) {
                        array.exchangeEntries(top, rightIndex);
    ...
    private static <T> Array<T> buildHeap(Queue<T> q, Comparator<T> order) {
         ...
         */
        Array<T> array = new Array1L<T>(q.length());
        for (int index = 0; index < q.length(); index++) {
            array.setEntry(index, q.dequeue());
        }
    ...
    private void createNewRep(Comparator<T> order) {
        ...
        this.heap = new Array1L<>(this.heapSize);
    }
