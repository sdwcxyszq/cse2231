...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private void createNewRep(int hashTableSize) {
        ...
        for (int index = 0; index < this.hashTable.length(); index++) {
            ...
            this.hashTable.setEntry(index, temp);
        }
    }
    ...
    public final Pair<K, V> remove(K key) {
        ...
        this.size--;
        int index = mod(key.hashCode(), this.hashTable.length());
        return this.hashTable.entry(index).remove(key);
    }

    @Override
    public final Pair<K, V> removeAny() {
        assert this.size() > 0 : "Violation of: this /= empty_set";
        Map.Pair<K, V> remove = null;
        int index = 0;
        boolean judge = false;
        while (!judge) {
            if (this.hashTable.entry(index).size() > 0) {
                judge = true;
                remove = this.hashTable.entry(index).removeAny();
            }
            index++;
        }
        this.size--;
    ...
    public final V value(K key) {
        ...
        assert this.hasKey(key) : "Violation of: key is in DOMAIN(this)";
        int index = mod(key.hashCode(), this.hashTable.length());
        return this.hashTable.entry(index).value(key);
    }
    ...
    public final boolean hasKey(K key) {
        ...
        int index = mod(key.hashCode(), this.hashTable.length());
        boolean judge = false;
        judge = this.hashTable.entry(index).hasKey(key);
        return judge;
    }
